    ...
    <>
          ...
          <HStack spacing={162}>
            <Box>
              <InputGroup size="md" ml="27.2rem">
                <Input w="14rem" bg="offWhite" placeholder="search collection..." onChange={event => setValue(event.target.value.toUpperCase())} value={value}/>
      ...
      <Box p="1rem">
        {loading?Images.map(post => (
          <Link to={{ pathname: '/All_Watches', hash: '', search: `Name=${post.seller}&img=${post.img}&des=${post.description}&name=${post.name}&price=${post.price}&inStock=${post.inStock}` }}>
