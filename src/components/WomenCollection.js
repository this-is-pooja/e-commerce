...
import { Flex, Center, Text, Box, Input, InputGroup, InputRightAddon, HStack, Image, Spacer } from "@chakra-ui/react";
...
function WomenCollection() {
  ...
  useEffect(() => {
    axios.get(`${BASE_URL}/products/women`)
      .then((res) => {
    ...
    <>
      ...
      <Box p="1rem">
        {loading?Images.map(post => (
          <Link to={{ pathname: '/WomenWatches', hash: '', search: `Name=${post.seller}&img=${post.img}&des=${post.description}&name=${post.name}&price=${post.price}&inStock=${post.inStock}` }}>
            <Box key={post.id} w="18.5rem" h="18rem" position="relative" style={{ borderRadius: "5px", boxShadow: "rgba(0, 0, 0, 0.15) 0px 3px 3px 0px" }} float="left" m="6.2rem">
              <Image src={post.img} borderRadius="4px" align="center" />
              <Box bg="offWhite" w="18.5rem" position="absolute" h="5rem">
                <Box p="0.8rem">
                  <Text color="lblack" fontSize="1rem" style={{ textTransform: "uppercase", letterSpacing: "0.1rem", wordSpacing: "0.1rem", whiteSpace: "nowrap", textOverflow: "ellipsis", overflow: "hidden" }}>{post.name}</Text>
                </Box>
